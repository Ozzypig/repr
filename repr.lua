...
local function repr(value, reprSettings)
	...
	elseif typeof then
		...
		elseif typeof(v) == "ColorSequenceKeypoint" then
			return ("ColorSequenceKeypoint.new(%g, %s)"):format(v.Time, repr(v.Value, reprSettings))
		...
		elseif typeof(v) == "NumberRange" then
			if v.Min == v.Max then
				return ("NumberRange.new(%g)"):format(v.Min)
			else
		...
		elseif typeof(v) == "NumberSequence" then
			if #v.Keypoints > 2 then
				...
				if v.Keypoints[1].Value == v.Keypoints[2].Value then
					return ("NumberSequence.new(%g)"):format(v.Keypoints[1].Value)
				else
					return ("NumberSequence.new(%g, %g)"):format(v.Keypoints[1].Value, v.Keypoints[2].Value)
				end
			end
		elseif typeof(v) == "NumberSequenceKeypoint" then
			if v.Envelope ~= 0 then
				return ("NumberSequenceKeypoint.new(%g, %g, %g)"):format(v.Time, v.Value, v.Envelope)
			else
				return ("NumberSequenceKeypoint.new(%g, %g)"):format(v.Time, v.Value)
			end
		...
		elseif typeof(v) == "PhysicalProperties" then
			return ("PhysicalProperties.new(%g, %g, %g, %g, %g)"):format(
				v.Density,
				v.Friction,
				v.Elasticity,
				v.FrictionWeight,
		...
		elseif typeof(v) == "Rect" then
			return ("Rect.new(%g, %g, %g, %g)"):format(v.Min.X, v.Min.Y, v.Max.X, v.Max.Y)
		...
		elseif typeof(v) == "Vector2int16" then
			return ("Vector2int16.new(%g, %g)"):format(v.X, v.Y)
		elseif typeof(v) == "Vector3" then
			return ("Vector3.new(%g, %g, %g)"):format(v.X, v.Y, v.Z)
		elseif typeof(v) == "Vector3int16" then
			return ("Vector3int16.new(%g, %g, %g)"):format(v.X, v.Y, v.Z)
